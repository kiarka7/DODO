...
def prepare_ligand(input_file, output_file, use_timeout, work_path):
...
def read_json(json_file, data_path, work_path):
    data = {}
...
def prepare_ligand_with_fallback(input_file, original_smiles_file, output_file, work_path):
    try:
        prepare_ligand(input_file, output_file, use_timeout=False, work_path=work_path)
...
def run_docking(json_file, data_path=DATA_PATH, work_path=WORK_PATH, output_folder="output", use_timeout=True):
    receptor, ligand, output, center_x, center_y, center_z, size_x, size_y, size_z, exhaustiveness = read_json(json_file, data_path, work_path)
    ...
    if ext_lower in ['.smi', '.mol2', '.cif', '.sdf', '.pdb']:
        ...
        try:
            prepare_ligand(ligand_pdb, ligand_pdbqt, use_timeout, work_path=work_path)
